...
define('DELAY', 60000);
function device_apply_conf(&$configuration)
{
  global $sdid;
  global $sms_sd_ctx;
  global $sms_sd_info;
  ...
  while ($line !== false)
  {
    ...
    if (strpos($line, '#') !== 0)
    {
      ...
      $index = sendexpect(__FILE__ . ':' . __LINE__, $sms_sd_ctx, $line, $tab[1], DELAY);
...
function device_update_apply_conf(&$configuration)
{
    global $sdid;
    global $sms_sd_ctx;
    global $sms_sd_info;
    ...
    while ($line !== false)
    {
        ...
        if (strpos($line, '#') == 0)
        {
            ...
            if (strpos($line, 'end') !== false || $line === '')
            {
                $tab[4] = '#';
            }
            elseif (strpos($line, '--More--') !== false )
	    {
		sendexpect(__FILE__ . ':' . __LINE__, $sms_sd_ctx, '\r', $tab[4], DELAY);
	 	$tab[4] = '#';			
	    }		 
            $index = sendexpect(__FILE__ . ':' . __LINE__, $sms_sd_ctx, $line, $tab[4], DELAY);
